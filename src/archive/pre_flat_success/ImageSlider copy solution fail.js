...
const ImageSlider = ({slides}) => {
    ...
    // overacheive loading
    const [nextSlideIndex, setNextSlideIndex] = useState(currentIndex + 1);
    const iframeRef = useRef(null);
    const preloadIframeRef = useRef(null);
    ...
    const goToNext= () => {
        ...
        setCurrentIndex(newIndex);
        setNextSlideIndex(newIndex + 1 < filteredSlides.length ? newIndex + 1 : 0);
    }
        ...
        <div className = 'sliderContainer'>
            ...
            <div className = 'rightArrow' onClick = {goToNext}>❱</div>
                {filteredSlides[currentIndex].type === 'jpg' ? (
                    ...
                    {filteredSlides[nextSlideIndex].type === "jpg" ? null : (
                        <iframe
                            className="preloadIframe"
                            src={filteredSlides[nextSlideIndex].url}
                            onLoad={() => (preloadIframeRef.current.style.display = "none")}
