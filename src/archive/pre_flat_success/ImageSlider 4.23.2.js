...
const ImageSlider = ({slides}) => {
    ...
    // to support over-achiever load which loads the next available iframe in all cases
    ...
    function findNextIframeIndex(currentIndex) {
        for (let i = currentIndex + 1; i < iframeRefs.current.length; i++) {
          if (iframeRefs.current[i].getAttribute('data-src')) {
            return i;
          }
        }
        return -1;
    ...
    function handleIframeLoad(index) {
        const nextIndex = findNextIframeIndex(index);
        if (nextIndex !== -1) {
          iframeRefs.current[nextIndex].src = iframeRefs.current[nextIndex].getAttribute('data-src');
        }
      }
